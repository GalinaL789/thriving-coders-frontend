...
const FormField = ({ errors = [], icons = [], ...props }: Props) => {
  ...
  const controlClassName = [
    ...
    errors && errors.length > 0 ? 'border-pink-600' : '',
  ].join(' ')
      ...
      <div className={`${elementWrapperClass}`}>
        {Children.map(props.children, (child: ReactElement, index) => (
          <div className="relative">
            {cloneElement(child, {
              className: `px-3 py-2 max-w-full rounded w-full dark:placeholder-gray-400
            focus:ring focus:ring-blue-600 focus:border-blue-600 focus:outline-none
            ${props.hasTextareaHeight ? 'h-24' : 'h-12'}
            ${props.isBorderless ? 'border-0' : 'border'}
            ${props.isTransparent ? 'bg-transparent' : 'bg-white dark:bg-slate-800'}
            ${errors && errors[index] ? 'border-pink-600' : 'border-gray-700'} 
            ${icons[index] ? 'pl-10' : ''}`,
            })}
            {icons[index] && (
              <Icon
            ...
            {errors && errors[index] && (
              <div className="text-sm text-red-500 dark:text-red-400 mt-1">{errors[index]}</div>
            )}
          </div>
        ))}
      </div>
